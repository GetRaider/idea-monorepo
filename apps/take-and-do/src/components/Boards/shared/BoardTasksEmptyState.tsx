...
import { ListBoardIcon } from "@/components/Icons/ListBoardIcon";
import { cn } from "@/lib/styles/utils";
import type { UiProps } from "@/lib/styles/ui-props";

export function BoardTasksEmptyState({
  boardName,
  quickCreateSlot,
  className,
}: BoardTasksEmptyStateProps) {
  return (
    <Outer className={className}>
      <GridBackdrop aria-hidden />
      <Inner>
        <IconCard>
          <ListBoardIcon size={28} className="text-[var(--text-primary)]" />
        </IconCard>
        <Title>Nothing on this board yet</Title>
        <Subtitle>
          Add your first task to start organizing work in{" "}
        ...
        {quickCreateSlot ? (
          <div className="mt-1 w-full max-w-lg">{quickCreateSlot}</div>
        ) : null}
      </Inner>
    </Outer>
  );
...
function Outer({ className, ref, ...props }: UiProps<"div">) {
  return (
    <div
      ref={ref}
      className={cn(
        "relative col-span-full flex min-h-[calc(100vh-200px)] w-full min-w-0 flex-col items-center justify-center px-6 py-12",
        className,
      )}
      {...props}
    />
...
function Inner({ className, ref, ...props }: UiProps<"div">) {
  return (
    <div
      ref={ref}
      className={cn(
        "relative z-[1] flex w-full max-w-md flex-col items-center gap-5 text-center",
        className,
      )}
      {...props}
    />
...
interface BoardTasksEmptyStateProps {
  boardName: string;
