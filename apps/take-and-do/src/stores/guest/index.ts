...
function parseStore(raw: string): GuestStore | null {
  try {
    ...
    const taskBoards = Array.isArray(record.taskBoards)
      ...
      : [];
    const sidebarOrder = parseSidebarOrder(record.sidebarOrder);
    return { tasks, folders, taskBoards, expiresAt, sidebarOrder };
  } catch {
    return null;
  }
}
...
export const guestStoreHelper = {
  ...
  getSidebarOrder(): GuestSidebarOrder | undefined {
    return this.read()?.sidebarOrder;
  },

  setSidebarOrder(order: GuestSidebarOrder): void {
    this._write((store) => ({ ...store, sidebarOrder: order }));
  },
