...
type TasksSidebarWorkspacesBodyProps = {
...
export function TasksSidebarWorkspacesBody({
  model,
  isFoldersLoading,
  isBoardsLoading,
}: TasksSidebarWorkspacesBodyProps) {
  ...
  const handleDragStart = useCallback(
    (event: DragStartEvent) => {
      ...
      if (data?.type === "sidebar-board") {
        const board = taskBoards.find((board) => board.id === data.boardId);
              ...
              <Fragment key={row.id}>
                ...
                {row.kind === "folder" ? (
                  <WorkspaceItem>
                    ...
                    >
                      <FolderRow isActive={openMenuFolderId === row.folder.id}>
                          ...
                          >
                            ...
                            <span className="flex min-w-0 flex-1 items-center gap-1.5">
                              <EmojiPickerField
                                ...
                                onToggle={() =>
                                  setOpenFolderEmojiPickerId((previous) =>
                                    previous === row.folder.id
                                      ? null
                                      : row.folder.id,
                                  )
                                }
                                onSelect={(emoji) => {
                                  ...
                                  void handleEditFolder(row.folder, { emoji });
                                }}
                                onClear={() => {
                        ...
                        {editingFolderId !== row.folder.id && (
                          <WorkspaceRowActions>
                            <Dropdown
                              ...
                              onOpenChange={(open) =>
                                setOpenMenuFolderId(open ? row.folder.id : null)
                              }
                            />
                          </WorkspaceRowActions>
                        )}
                      ...
                      boardsInFolderSorted(row.folder.id).length > 0 ? (
                        <SubItems
                          isDropActive={folderListDropHighlight(row.folder.id)}
                        >
